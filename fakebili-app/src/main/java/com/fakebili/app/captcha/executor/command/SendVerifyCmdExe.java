...
public class SendVerifyCmdExe {
    ...
    public void execute(SendVerifyCmd cmd) {
        ...
        redisService.set(cmd.getEmail(), code, 5 * 60);
        // TODO 发布邮件事件
        // TODO type 用枚举的key
        this.applicationContext.publishEvent(new SendVerifyEvent(TextVerifyEntity.builder().type(0).code(code).email(cmd.getEmail()).build()));
    }
}
